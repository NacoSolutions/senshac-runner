...
`devenv.nix` and lock remain the package contract and are checked
before the Nix build. It is the only devenv source contract for non-base tools.

## Responsibilities

- **devenv** owns the developer environment, package manifest, lockfile, custom
  Senshac tools, and activation contract. Changes to
  `devenv.nix` continue to require the matching lockfile.
- **Nix** owns the reproducible published base closure in `flake.nix`, including
  the pinned devenv CLI/runtime package and the `dockerTools` OCI artifact. Bun,
  Chromium, and every other project tool remain owned by the mounted devenv
  manifest/lock. It has no Dockerfile, `FROM` image, apt-get step, or base
  distribution layer.

The image closure contains only the base shell/runtime, the pinned devenv
...
project's committed `devenv.lock`; no hand-maintained tool list is
duplicated in the flake. The image build never evaluates the developer-only
custom devenv inputs (Canopy, Cloudflare CLI, Knip, Mulch, Seeds, Terrarium, or
...
The entrypoint contains the devenv CLI and always activates the mounted project.
It requires `devenv.lock`, runs `devenv shell`, and never installs
...
are required by devenv's activation interpreter contract. The
`scripts/verify-nix-base IMAGE` check validates both paths before mounting this
repository and verifying `devenv shell --
command -v bun`, `devenv shell -- command -v chromium`, and a headless
Chromium launch. Prefer a pre-warmed devenv/Nix cache in CI; the image itself
...
The flake pins the released devenv flake and declares devenv's official
`cache.devenv.dev` substituter and public key through `nixConfig`. The validation
scripts pass `--accept-flake-config`, which is required on generic Nix hosts;
this prevents the devenv CLI and patched Nix Rust closure from being built from
...
The expected closure contains the pinned devenv runtime but no project package
paths. Custom devenv inputs remain in the committed project lock for developer
...
devenv and Nix smoke contracts. The image uses an activation wrapper so the smoke script exercises the
...
intentionally does not include devenv-only Seeds, Mulch, Terrarium, Trellis, or
other interactive developer tooling; those remain available through the
explicit mounted devenv shell activation contract.
...
its configured binary cache or build on a cache miss. devenv shell activation uses the
