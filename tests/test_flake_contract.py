...
class FlakeContractTests(unittest.TestCase):
    def test_devenv_and_docker_tools_contract(self):
        ...
        self.assertIn('inputs.devenv.url = "github:cachix/devenv/v1.8.1"', FLAKE)
        self.assertIn("devenv.packages.${pkgs.system}.devenv", FLAKE)
        self.assertIn("devenv.lock", FLAKE)
        self.assertNotIn(" jq devenv", FLAKE)
